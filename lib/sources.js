 ...
 *
 * Resolution order:
 *   1. user-added custom sources (settings `customDocs`, exact name match)
 *   2. curated registry entries carrying a verified llms.txt URL
 *   3. the npm metadata degradation chain (llms-full.txt → llms.txt → README)
 *
...
export async function resolveLibrary(libraryName, { customDocs = [] } = {}) {
  ...
  const curated = registryEntry(libraryName)
  if (curated?.url) {
    return {
      name: libraryName,
      version: null,
      description: 'curated registry docs source',
      curated: true,
      sources: [{ type: 'curated-llms', url: curated.url }],
    }
  }

 ...
 */
export async function resolveLibrary(libraryName) {
...
/**
 * Rebuild the source list pinned to an exact version.
 * GitHub raw URLs are tried with "v<version>" and "<version>" tag refs first,
 * then fall back to the unpinned chain.
 */
export function sourcesForVersion(resolved, version) {
  ...
  const seen = new Set()
  return [...pinned, ...resolved.sources].filter((s) => {
